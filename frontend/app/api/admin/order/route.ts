...
export async function GET(req: Request) {
  try {
    ...
    const totalProcessingOrders = await Order.find({
      status: "shipped",
    }).countDocuments();
    ...
    const totalMoney =
      totalMoneyforTodaysOrders.length > 0
        ? totalMoneyforTodaysOrders[0].total
        : 0;
    ...
    const totalMoneyThisMonth =
      totalMoneyforThisMonthOrders.length > 0
        ? totalMoneyforThisMonthOrders[0].totalMoney
        : 0;
    ...
    const totalMoneyTillNowValue =
      totalMoneyTillNow.length > 0 ? totalMoneyTillNow[0].totalMoney : 0;
