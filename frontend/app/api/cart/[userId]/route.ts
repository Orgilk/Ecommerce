...
export async function GET(
  ...
  try {
    ...
    productsWithDetails = productsWithDetails.filter(
      (item: any) => item !== null
    );
    ...
    for (let i = 0; i < productsWithDetails.length; i++) {
      totalPrice +=
        productsWithDetails[i].price * productsWithDetails[i].quantity;
    }

    return NextResponse.json(
      { success: true, data: productsWithDetails, totalPrice: totalPrice },
      { status: 200 }
    );
  } catch (error) {
    return NextResponse.json(
