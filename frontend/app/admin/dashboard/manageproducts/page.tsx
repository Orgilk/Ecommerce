...
const ManageProducts = () => {
  ...
  const [selectedPriceRange, setSelectedPriceRange] =
    useState<PriceFilterOption>("all");
  ...
  const filterProductsByPrice = (product: Product) => {
    switch (selectedPriceRange) {
      ...
      case "medium":
        return (
          product.price > priceRanges.low && product.price <= priceRanges.medium
        );
      case "high":
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <h1 className="text-3xl font-bold text-gray-900">Бараа удирдах</h1>
        <motion.button
                    ...
                    <div className="flex flex-wrap gap-2">
                      {["all", "low", "medium", "high"].map((range) => (
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          onClick={() =>
                            setSelectedPriceRange(range as PriceFilterOption)
                          }
                ...
                <CardContent className="flex-1 p-4 space-y-3">
                  <div>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={`shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusStyle(
                          product.status || ""
                        )}`}
                      >
                        {product.status}
