...
const ProductDetail = ({ products }: { products: Product[] }) => {
  ...
  const allSizes = useMemo(
    () =>
      Array.from(new Set(products.flatMap((product) => product.size || []))),
    [products]
  );
  const allColors = useMemo(
    () =>
      Array.from(new Set(products.flatMap((product) => product.color || []))),
    [products]
  );
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      const matchesColor =
        selectedColor === "All" || product.color?.includes(selectedColor);
      const matchesPrice =
          ...
          <ul className="list-disc pl-5">
            {filteredProducts.slice(0, 5).map((product) => (
              <li key={product.id} className="text-sm text-gray-600">
                <Link
                  href={`/admin/dashboard/category/${product.category}/${product.id}`}
                >
